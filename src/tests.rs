...
fn number_range_1() {
    let data: Vec<(&str, NumberRange, &[usize], std::result::Result<(), (IsWithinErrorType, String)>, std::result::Result<(), String>)> = vec![
        ...
        ("[(0, (1, 100))]", NumberRange::all(1, 100), &[1, 99, 4, 6, 101], Err((IsWithinErrorType::Regular, "Selected number 101 at index 4 is not within range of min: 1 and max: 100. Numbers:[1, 99, 4, 6, 101].  Map Index:0".to_owned())), Err("Selected number 101 at index 4 is not within range of min: 1 and max: 100. Numbers:[1, 99, 4, 6, 101].  Map Index:0".to_owned())),
